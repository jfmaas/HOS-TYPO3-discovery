...
$(function() {
        ...
        // Preparing DOM
        $('.facet-id-Fachgebiet').html('<h1>Fachgebietsbaum (DDC)</h1><div id="ddctree" style="width:100%;"/>');
        ...
        $('.facet-id-Fachgebiet .facetList').show(1000); 
...
function FacetsToTree(DDC,facets,preselectedDDC) {
	var DDC_LINK = '?id=1&tx_find_find[facet][Fachgebiet][###NEEDLE###]=1&tx_find_find[controller]=Search'; 
	var data = [];
	...
	Object.keys(facets).forEach(function(code){
		...
		if (!level2Node) {	
			level1Node.children.push({
				a_attr : {href:DDC_LINK.replace('###NEEDLE###',level2)},
				name:level2,
		...
		if (level3) {
			level2Node.children.push({
				 a_attr : {href:DDC_LINK.replace('###NEEDLE###',level3)},
				 name:level3,	
